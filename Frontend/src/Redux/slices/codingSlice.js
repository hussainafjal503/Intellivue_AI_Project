  ...
  reducers: {
    ...
    genrateQuestionSuccess: (state, action) => {
      ...
      sessionStorage.setItem(
        "question",
        JSON.stringify(action.payload.data.codeQuestion)
      );
    },
...
export const getAllQuestion = () => async (dispatch) => {
  ...
  try {
    ...
    // console.log(response);
    dispatch(
      codingSlice.actions.getAllQuestionSuccess(
...
export const getSingleQuestion = (data) => async (dispatch) => {
  ...
  try {
    ...
    // console.log(response);
    dispatch(
      codingSlice.actions.getSingleQuestionSuccess(response?.data?.data)
    );
    dispatch(codingSlice.actions.clearAllErrorRequest());
  } catch (err) {
    console.log(`Error occured while geting single question data : ${err}`);
    dispatch(
      codingSlice.actions.getSingQuestionFailed(err?.response?.data?.message)
    );
  }
};
