...
class UserController {
  ...
  updatePasswordController = async (req, res, next) => {
    try {
      const response = await serviceUser.updatePasswordService(
        req.user.id,
        req.body
      );
  ...
  updateProfileController = async (req, res, next) => {
    try {
      const response = await serviceUser.updateProfileService(
        req.user.id,
        req.body
      );
  ...
  updateAvtarController = async (req, res, next) => {
    try {
      ...
      const response = await serviceUser.updateAvtarService(
        req.user.id,
        req.file
      );
      if (response.success === false) {
  ...
  sendAccountDeleteRequestController = async (req, res, next) => {
    try {
      const response = await serviceUser.sendAccountDeleteRequestService(
        req.user.id
      );
      if (response.statusCode === 400) {
        return next(new CustomError(response.message, response.statusCode));
      ...
      console.log(
        `Error Occured in send Account Delete Request Controller : ${err}`
      );
